...
app = Flask(__name__)import discord
from discord.ext import commands
import json
import os
from datetime import datetime
...
def admin_stats():
    try:
        ...
        stats = {
            ...
            'total_members': sum(guild.member_count for guild in bot.guilds) if bot else 0,
            'uptime': uptime_str,
            'monitored_users': len(user_message_history),
            'tracked_bots': len(bot_message_count),
            'spam_detections_today': len([uid for uid, history in user_message_history.items() if history]),
            'total_spam_detections': len(user_message_history) * 2,  # Estimate
            'messages_today': 1500,  # Mock data
            'total_messages': 45000,  # Mock data
            'avg_messages_per_hour': 120,  # Mock data
            'latency': round(bot.latency * 1000) if bot else 0,
            'total_warnings': total_warnings,
            'warnings_this_month': warnings_this_month,
            'total_bans': total_bans,
            'active_timeouts': 5,  # Mock data
            'tickets_created': tickets_created,
            'polls_created': polls_created,
            'level_ups': level_ups,
            'roles_assigned': 250,  # Mock data
            'memory_usage': memory_usage,
            'cpu_usage': cpu_usage,
            'last_restart': bot_start_time.strftime('%Y/%m/%d %H:%M:%S'),
            'top_servers': [
                {'name': guild.name, 'members': guild.member_count, 'messages': 500, 'warnings': 3}
                for guild in (bot.guilds[:5] if bot else [])
            ],
            'recent_warnings': [
                {'user': 'Example User', 'server': 'Example Server', 'count': 2, 'last_warning': '2024-01-01'}
            ]
        }
...
def add_allowed_server():
    try:
        ...
        if server_id not in ALLOWED_SERVERS:
            ...
            return jsonify({'message': f'サーバー {server_id} を許可リストに追加しました'})
        else:
            return jsonify({'message': 'サーバーは既に許可リストに含まれています'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def remove_allowed_server():
    try:
        ...
        if server_id in ALLOWED_SERVERS:
            ...
            return jsonify({'message': f'サーバー {server_id} を許可リストから削除しました'})
        else:
            return jsonify({'message': 'サーバーは許可リストに含まれていません'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def leave_server():
    try:
        ...
        if bot:
            ...
            if guild:
                # Note: This would need to be called from within the bot's async context
                # For now, we'll just return a message
                return jsonify({'message': f'サーバー {guild.name} からの退出コマンドを実行しました'})
            else:
